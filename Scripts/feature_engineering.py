...
# Drop outliers from each game_url based on odds columns using a loop
z_thresh = 3
for index, group in tqdm(odds.groupby('game_url')):
    for col in ['opening_odds1', 'opening_odds2', 'closing_odds1', 'closing_odds2']:
        z_scores = np.abs((group[col] - group[col].mean()) / group[col].std())
        mask = z_scores < z_thresh
        odds.loc[group.index, col] = group[col].where(mask)
...
odds = odds.sort_values(['game_url', 'opening_time']).drop_duplicates(subset = 'game_url').set_index('game_url').drop(columns=['player1', 'player2', 'comment', 'bookmaker', 'opening_odds1', 'opening_odds2', 'closing_odds1', 'closing_odds2'])
...
games.insert(games.columns.get_loc('score'), 'comment', np.where(games['score'].isna(), 'No Data',
                                                            np.where(games['score'].str.contains(r'\b(RET|RE|Ret)\b', na=False), 'Retired',
                                                                np.where(games['score'].str.contains(r'\b(W/O|Walkover)\b', na=False), 'Walkover',
                                                                    np.where(games['score'].str.contains(r'\b(DEF|Def|Default)\b', na=False), 'Default',
                                                                        np.where(games['score'].str.contains(r'\b(UNK|UNP)\b', na=False), 'Unknown',
                                                                            np.where(games['score'].str.contains(r'\b(abandoned|ABD|unfinished|>)\b', na=False), 'Abandoned', 'Completed')))))))
games = games[(games['comment'] == 'Completed')].reset_index(drop=True)
...
games['w_mean_closing_odds'] = games.apply(lambda row: row['odds1'] if row['player1'] == row['winner'] else row['odds2'], axis=1)
games['l_mean_closing_odds'] = games.apply(lambda row: row['odds2'] if row['player1'] == row['winner'] else row['odds1'], axis=1)
...
#games[games['w_sets'] < 2][['winner', 'loser', 'season', 'tourney_name', 'round', 'comment', 'score', 'w_sets', 'l_sets', 'w_games', 'l_games']].reset_index(drop=True)
games = games[games['w_sets'] >= 2].reset_index(drop=True)
...
class CalculateElo:
    ...
    # Update Elo ratings
    def updateElo(self, playerA, playerB, level, match_date, w_games, l_games):
        ...
        g = (w_games / (w_games + l_games))
        
        rA_new = rA + (k * kA) * (sA - eA) * g
        rB_new = rB + (k * kB) * (sB - eB) * g

        self.playersToElo.setdefault(playerA, []).append((rA_new, match_date))
        self.playersToElo.setdefault(playerB, []).append((rB_new, match_date))
        return rA, rB
    ...
    def calculate_elo(self):
        if self.surface == None:
            for index, row in tqdm(self.games.iterrows(), total = len(self.games), desc = f'Getting Elo'):
                ...
                self.elo_df.loc[index, ['winner_elo', 'loser_elo']] = self.updateElo(playerA, playerB, row["tourney_level"], row["tourney_date"], row['w_games'], row['l_games'])
        else:
            ...
            for index, row in tqdm(self.games.iterrows(), total = len(self.games), desc = f'Getting Elo {self.surface}'):
                playerA, playerB = row["winner_name"], row["loser_name"]
                self.updateMatchesCount(playerA, playerB)
                self.elo_df.loc[index, [f'winner_elo_{self.surface}', f'loser_elo_{self.surface}']] = self.updateElo(playerA, playerB, row["tourney_level"], row["tourney_date"], row['w_games'], row['l_games'])
...
matchesCount = {}
elo_df = pd.DataFrame()
test = games[games['season'] >= 2015].copy()

# Update matches count
def updateMatchesCount(playerA, playerB):
    matchesCount[playerA] = matchesCount.get(playerA, 0) + 1
    matchesCount[playerB] = matchesCount.get(playerB, 0) + 1

# Update Elo ratings
def updateElo(playerA, playerB, level, match_date, w_games, l_games):
    rA = playersToElo.get(playerA, [(1500, datetime(1900, 1, 1), 0)])[-1][0]
    rB = playersToElo.get(playerB, [(1500, datetime(1900, 1, 1), 0)])[-1][0]
    ...
    playersToElo.setdefault(playerA, []).append((rA_new, match_date))
    playersToElo.setdefault(playerB, []).append((rB_new, match_date))
    return rA, rB

for index, row in tqdm(test.iterrows(), total = len(test), desc = f'Getting Elo'):
    playerA, playerB = row["winner_name"], row["loser_name"]
    updateMatchesCount(playerA, playerB)
    elo_df.loc[index, ['winner_elo', 'loser_elo']] = updateElo(playerA, playerB, row["tourney_level"], row["tourney_date"], row['w_games'], row['l_games'])
"""
...
games['tourney_name'].replace({'ATP Rio de Janeiro':'Rio de Janeiro', 'Rio De Janeiro':'Rio de Janeiro', 'Adelaide 1':'Adelaide', 'Adelaide 2':'Adelaide',
...
games['favored_win'] = games.apply(lambda row: 1 if row['winner_elo'] > row['loser_elo'] else 0, axis=1)

games['favored'] = games.apply(lambda row: row['winner_name'] if row['winner_elo'] > row['loser_elo'] else row['loser_name'], axis=1)
games['underdog'] = games.apply(lambda row: row['loser_name'] if row['winner_elo'] > row['loser_elo'] else row['winner_name'], axis=1)

games['favored_id'] = games.apply(lambda row: row['winner_id'] if row['winner_elo'] > row['loser_elo'] else row['loser_id'], axis=1)
games['underdog_id'] = games.apply(lambda row: row['loser_id'] if row['winner_elo'] > row['loser_elo'] else row['winner_id'], axis=1)

games['favored_entry'] = games.apply(lambda row: row['winner_entry'] if row['winner_elo'] > row['loser_elo'] else row['loser_entry'], axis=1)
games['underdog_entry'] = games.apply(lambda row: row['loser_entry'] if row['winner_elo'] > row['loser_elo'] else row['winner_entry'], axis=1)

games['favored_seed'] = games.apply(lambda row: row['winner_seed'] if row['winner_elo'] > row['loser_elo'] else row['loser_seed'], axis=1)
games['underdog_seed'] = games.apply(lambda row: row['loser_seed'] if row['winner_elo'] > row['loser_elo'] else row['winner_seed'], axis=1)

games['favored_hand'] = games.apply(lambda row: row['winner_hand'] if row['winner_elo'] > row['loser_elo'] else row['loser_hand'], axis=1)
games['underdog_hand'] = games.apply(lambda row: row['loser_hand'] if row['winner_elo'] > row['loser_elo'] else row['winner_hand'], axis=1)

games['favored_ht'] = games.apply(lambda row: row['winner_ht'] if row['winner_elo'] > row['loser_elo'] else row['loser_ht'], axis=1)
games['underdog_ht'] = games.apply(lambda row: row['loser_ht'] if row['winner_elo'] > row['loser_elo'] else row['winner_ht'], axis=1)

games['favored_ioc'] = games.apply(lambda row: row['winner_ioc'] if row['winner_elo'] > row['loser_elo'] else row['loser_ioc'], axis=1)
games['underdog_ioc'] = games.apply(lambda row: row['loser_ioc'] if row['winner_elo'] > row['loser_elo'] else row['winner_ioc'], axis=1)

games['favored_age'] = games.apply(lambda row: row['winner_age'] if row['winner_elo'] > row['loser_elo'] else row['loser_age'], axis=1)
games['underdog_age'] = games.apply(lambda row: row['loser_age'] if row['winner_elo'] > row['loser_elo'] else row['winner_age'], axis=1)

games['favored_rank'] = games.apply(lambda row: row['winner_rank'] if row['winner_elo'] > row['loser_elo'] else row['loser_rank'], axis=1)
games['underdog_rank'] = games.apply(lambda row: row['loser_rank'] if row['winner_elo'] > row['loser_elo'] else row['winner_rank'], axis=1)

games['favored_rank_group'] = games.apply(lambda row: row['winner_rank_group'] if row['winner_elo'] > row['loser_elo'] else row['loser_rank_group'], axis=1)
games['underdog_rank_group'] = games.apply(lambda row: row['loser_rank_group'] if row['winner_elo'] > row['loser_elo'] else row['winner_rank_group'], axis=1)

games['favored_elo'] = games.apply(lambda row: row['winner_elo'] if row['winner_elo'] > row['loser_elo'] else row['loser_elo'], axis=1)
games['underdog_elo'] = games.apply(lambda row: row['loser_elo'] if row['winner_elo'] > row['loser_elo'] else row['winner_elo'], axis=1)

games['favored_elo_surface'] = games.apply(lambda row: row['winner_elo_surface'] if row['winner_elo'] > row['loser_elo'] else row['loser_elo_surface'], axis=1)
games['underdog_elo_surface'] = games.apply(lambda row: row['loser_elo_surface'] if row['winner_elo'] > row['loser_elo'] else row['winner_elo_surface'], axis=1)

games['favored_sets'] = games.apply(lambda row: row['w_sets'] if row['winner_elo'] > row['loser_elo'] else row['l_sets'], axis=1)
games['underdog_sets'] = games.apply(lambda row: row['l_sets'] if row['winner_elo'] > row['loser_elo'] else row['w_sets'], axis=1)

games['favored_games'] = games.apply(lambda row: row['w_games'] if row['winner_elo'] > row['loser_elo'] else row['l_games'], axis=1)
games['underdog_games'] = games.apply(lambda row: row['l_games'] if row['winner_elo'] > row['loser_elo'] else row['w_games'], axis=1)

games['favored_ace'] = games.apply(lambda row: row['w_ace'] if row['winner_elo'] > row['loser_elo'] else row['l_ace'], axis=1)
games['underdog_ace'] = games.apply(lambda row: row['l_ace'] if row['winner_elo'] > row['loser_elo'] else row['w_ace'], axis=1)

games['favored_df'] = games.apply(lambda row: row['w_df'] if row['winner_elo'] > row['loser_elo'] else row['l_df'], axis=1)
games['underdog_df'] = games.apply(lambda row: row['l_df'] if row['winner_elo'] > row['loser_elo'] else row['w_df'], axis=1)

games['favored_svpt'] = games.apply(lambda row: row['w_svpt'] if row['winner_elo'] > row['loser_elo'] else row['l_svpt'], axis=1)
games['underdog_svpt'] = games.apply(lambda row: row['l_svpt'] if row['winner_elo'] > row['loser_elo'] else row['w_svpt'], axis=1)

games['favored_1st_in'] = games.apply(lambda row: row['w_1stIn'] if row['winner_elo'] > row['loser_elo'] else row['l_1stIn'], axis=1)
games['underdog_1st_in'] = games.apply(lambda row: row['l_1stIn'] if row['winner_elo'] > row['loser_elo'] else row['w_1stIn'], axis=1)

games['favored_1st_won'] = games.apply(lambda row: row['w_1stWon'] if row['winner_elo'] > row['loser_elo'] else row['l_1stWon'], axis=1)
games['underdog_1st_won'] = games.apply(lambda row: row['l_1stWon'] if row['winner_elo'] > row['loser_elo'] else row['w_1stWon'], axis=1)

games['favored_2nd_won'] = games.apply(lambda row: row['w_2ndWon'] if row['winner_elo'] > row['loser_elo'] else row['l_2ndWon'], axis=1)
games['underdog_2nd_won'] = games.apply(lambda row: row['l_2ndWon'] if row['winner_elo'] > row['loser_elo'] else row['w_2ndWon'], axis=1)

games['favored_serve_games'] = games.apply(lambda row: row['w_SvGms'] if row['winner_elo'] > row['loser_elo'] else row['l_SvGms'], axis=1)
games['underdog_serve_games'] = games.apply(lambda row: row['l_SvGms'] if row['winner_elo'] > row['loser_elo'] else row['w_SvGms'], axis=1)

games['favored_svpt_won'] = games.apply(lambda row: row['w_svpt_won'] if row['winner_elo'] > row['loser_elo'] else row['l_svpt_won'], axis=1)
games['underdog_svpt_won'] = games.apply(lambda row: row['l_svpt_won'] if row['winner_elo'] > row['loser_elo'] else row['w_svpt_won'], axis=1)

games['favored_rtpt_won'] = games.apply(lambda row: row['w_rtpt_won'] if row['winner_elo'] > row['loser_elo'] else row['l_rtpt_won'], axis=1)
games['underdog_rtpt_won'] = games.apply(lambda row: row['l_rtpt_won'] if row['winner_elo'] > row['loser_elo'] else row['w_rtpt_won'], axis=1)

games['favored_bp_saved'] = games.apply(lambda row: row['w_bpSaved'] if row['winner_elo'] > row['loser_elo'] else row['l_bpSaved'], axis=1)
games['underdog_bp_saved'] = games.apply(lambda row: row['l_bpSaved'] if row['winner_elo'] > row['loser_elo'] else row['w_bpSaved'], axis=1)

games['favored_bp_faced'] = games.apply(lambda row: row['w_bpFaced'] if row['winner_elo'] > row['loser_elo'] else row['l_bpFaced'], axis=1)
games['underdog_bp_faced'] = games.apply(lambda row: row['l_bpFaced'] if row['winner_elo'] > row['loser_elo'] else row['w_bpFaced'], axis=1)

games['favored_bp_won'] = games.apply(lambda row: row['w_bpWon'] if row['winner_elo'] > row['loser_elo'] else row['l_bpWon'], axis=1)
games['underdog_bp_won'] = games.apply(lambda row: row['l_bpWon'] if row['winner_elo'] > row['loser_elo'] else row['w_bpWon'], axis=1)
...
games['favored_odds'] = games.apply(lambda row: row['w_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_opening_odds'], axis=1)
games['underdog_odds'] = games.apply(lambda row: row['l_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_opening_odds'], axis=1)

games['favored_mean_opening_odds'] = games.apply(lambda row: row['w_mean_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_mean_opening_odds'], axis=1)
games['underdog_mean_opening_odds'] = games.apply(lambda row: row['l_mean_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_mean_opening_odds'], axis=1)

games['favored_mean_closing_odds'] = games.apply(lambda row: row['w_mean_closing_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_mean_closing_odds'], axis=1)
games['underdog_mean_closing_odds'] = games.apply(lambda row: row['l_mean_closing_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_mean_closing_odds'], axis=1)

games['favored_max_opening_odds'] = games.apply(lambda row: row['w_max_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_max_opening_odds'], axis=1)
games['underdog_max_opening_odds'] = games.apply(lambda row: row['l_max_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_max_opening_odds'], axis=1)

games['favored_max_closing_odds'] = games.apply(lambda row: row['w_max_closing_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_max_closing_odds'], axis=1)
games['underdog_max_closing_odds'] = games.apply(lambda row: row['l_max_closing_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_max_closing_odds'], axis=1)

games['favored_min_opening_odds'] = games.apply(lambda row: row['w_min_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_min_opening_odds'], axis=1)
games['underdog_min_opening_odds'] = games.apply(lambda row: row['l_min_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_min_opening_odds'], axis=1)

games['favored_min_closing_odds'] = games.apply(lambda row: row['w_min_closing_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_min_closing_odds'], axis=1)
games['underdog_min_closing_odds'] = games.apply(lambda row: row['l_min_closing_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_min_closing_odds'], axis=1)

games['favored_pinnacle_opening_odds'] = games.apply(lambda row: row['w_pinnacle_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_pinnacle_opening_odds'], axis=1)
games['underdog_pinnacle_opening_odds'] = games.apply(lambda row: row['l_pinnacle_opening_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_pinnacle_opening_odds'], axis=1)

games['favored_pinnacle_closing_odds'] = games.apply(lambda row: row['w_pinnacle_closing_odds'] if row['winner_elo'] > row['loser_elo'] else row['l_pinnacle_closing_odds'], axis=1)
games['underdog_pinnacle_closing_odds'] = games.apply(lambda row: row['l_pinnacle_closing_odds'] if row['winner_elo'] > row['loser_elo'] else row['w_pinnacle_closing_odds'], axis=1)
...
# Surface clusters
tourney_surface_speed = games.drop_duplicates(subset = ['tourney_name', 'season']).groupby(['tourney_name', 'surface'])['surface_speed'].mean().reset_index().dropna().sort_values(by='surface_speed')
tourney_surface_speed['surface_code'] = tourney_surface_speed['surface'].map({'Hard': 0, 'Clay': 1, 'Grass': 2})

X = tourney_surface_speed[['surface_code', 'surface_speed']].values
#scaler = StandardScaler()
#X_scaled = scaler.fit_transform(X)

kmeans = KMeans(n_clusters=6, random_state=42)
tourney_surface_speed['cluster'] = kmeans.fit_predict(X)

plt.figure(figsize=(8, 5))
sns.scatterplot(
    data=tourney_surface_speed,
    x='surface_speed',
    y='surface',
    hue='cluster',
    palette='Set1'
)
plt.title('Tournament Clusters by Surface and Surface Speed')
plt.xlabel('Surface Speed')
plt.ylabel('Surface')
plt.legend(title='Cluster')
plt.show()

print(tourney_surface_speed[['tourney_name', 'surface', 'surface_speed', 'cluster']].sort_values('cluster'))
tourney_surface_speed['cluster'].replace({0:'slow_hard', 1:'grass', 2:'slow_clay', 3:'fast_hard', 4:'fast_clay', 5:'intermediate_clay'}, inplace=True)
tourney_surface_speed.rename(columns = {'cluster':'surface_cluster'}, inplace=True)

games = pd.merge(games, tourney_surface_speed[['tourney_name', 'surface', 'surface_cluster']], on = ['tourney_name', 'surface'], how = 'left')
surface_speed_col = games.pop('surface_cluster')
games.insert(games.columns.get_loc('surface')+1, 'surface_cluster', surface_speed_col)
...
for index, row in tqdm(games_to_process.iterrows(), total=len(games_to_process)):
    ...
    favored_df['time_weight'] /= favored_df['time_weight'].sum()
    underdog_df['time_weight'] /= underdog_df['time_weight'].sum()
    
    # Surface weights
    surface_correlation = surface_correlation_matrix.loc[row['surface']]
    surface_correlation = surface_correlation / surface_correlation.sum()
    
    favored_surface_counts = favored_df['surface'].map(favored_df['surface'].value_counts())
    underdog_surface_counts = underdog_df['surface'].map(underdog_df['surface'].value_counts())
    
    favored_df['surface_weight'] = favored_df['surface'].map(surface_correlation).fillna(0) / favored_surface_counts
    underdog_df['surface_weight'] = underdog_df['surface'].map(surface_correlation).fillna(0) / underdog_surface_counts
    
    # Overall weights
    favored_df['weight'] = favored_df['time_weight'] * favored_df['surface_weight']
    underdog_df['weight'] = underdog_df['time_weight'] * underdog_df['surface_weight']
    
    favored_df['weight'] /= favored_df['weight'].sum()
    underdog_df['weight'] /= underdog_df['weight'].sum()
    ...
    features.loc[index, 'uncertainty'] = 1/(1+len(favored_df)) + 1/(1+len(underdog_df))
    ...
    # Surface Record
    filtered_favored_df = favored_df[(favored_df['surface'] == row['surface']) & (favored_df['tourney_date'] < row['tourney_date']) & (favored_df['round'] > 'Q3')]
    features.loc[index, 'favored_win_pct_surface'] = np.dot(filtered_favored_df['win'], filtered_favored_df['weight']/sum(filtered_favored_df['weight']))
    
    filtered_underdog_df = underdog_df[(underdog_df['surface'] == row['surface']) & (underdog_df['tourney_date'] < row['tourney_date']) & (underdog_df['round'] > 'Q3')]
    features.loc[index, 'underdog_win_pct_surface'] = np.dot(filtered_underdog_df['win'], filtered_underdog_df['weight']/sum(filtered_underdog_df['weight']))
    
    # Tourney series record
    filtered_favored_df = favored_df[(favored_df['tourney_series'] == row['tourney_series']) & (favored_df['tourney_date'] < row['tourney_date']) & (favored_df['round'] > 'Q3')]
    features.loc[index, 'favored_win_pct_tourney_series'] = np.dot(filtered_favored_df['win'], filtered_favored_df['weight']/sum(filtered_favored_df['weight']))
    
    filtered_underdog_df = underdog_df[(underdog_df['tourney_series'] == row['tourney_series']) & (underdog_df['tourney_date'] < row['tourney_date']) & (underdog_df['round'] > 'Q3')]
    features.loc[index, 'underdog_win_pct_tourney_series'] = np.dot(filtered_underdog_df['win'], filtered_underdog_df['weight']/sum(filtered_underdog_df['weight']))
    
    # Tourney record
    filtered_favored_df = favored_df[(favored_df['tourney_name'] == row['tourney_name']) & (favored_df['tourney_date'] < row['tourney_date']) & (favored_df['round'] > 'Q3')]
    ...
    filtered_underdog_df = underdog_df[(underdog_df['tourney_name'] == row['tourney_name']) & (underdog_df['tourney_date'] < row['tourney_date']) & (underdog_df['round'] > 'Q3')]
...
features['win_pct_surface_diff'] = features['favored_win_pct_surface'] - features['underdog_win_pct_surface']
features['win_pct_tourney_series_diff'] = features['favored_win_pct_tourney_series'] - features['underdog_win_pct_tourney_series']
